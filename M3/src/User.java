...
public class User {
	...
	public int getPlanetasCount(Connection con) {
		String query = "SELECT count(planet_name) FROM bbdd_planetwars.planets where id_user = " + getId_user() + ";";
		ResultSet rs = null;
		int resultado = 0;
		try {
	...
	public String[] getPlanetasListado(Connection con, int totalPlanets) {
		...
		String query = "SELECT planet_name FROM bbdd_planetwars.planets where id_user = " + getId_user() +";";
		ResultSet rs = null;
	...
	public void crearPlaneta(Connection con, String planet_name, int technologyAtack, int technologyDefense,
		...
		int idNewPlanet = 0; 
		ResultSet rs = null;
		rs = DB_utils.SelectQuery(con, "SELECT max(id_planet)+1 FROM bbdd_planetwars.planets;");
